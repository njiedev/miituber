...
export interface TourStep {
  ...
  text: string;
  emotion: TourEmotion;
  /** Exact substring of `text` rendered in the accent color, e.g. "Window Capture". */
  highlight?: string;
}
...
export const TOUR_CHAPTERS: Record<TourChapterId, TourChapter> = {
  ...
  workspace: {
    ...
    steps: [
      ...
      {
        id: "ws-signals",
        target: '.rail-item[data-menu="advanced"]',
        emotion: "normal",
        text: "Advanced hides the Expression Signals lab — tune when each expression kicks in with enter, exit, and gain sliders.",
      },
      {
        id: "ws-output",
        target: "#isolate-capture-button",
        emotion: "normal",
        text: "When you're ready to stream, Clean View gives OBS a clean window — add it with Window Capture and the background stays truly transparent.",
