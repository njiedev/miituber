...
### Added
- **First-run onboarding tour** with separate library and workspace chapters, a replay `?` button, typewriter dialogue, Mii portrait frame-flips, optional Mii SFX, and a spotlight overlay. Completion is stored per chapter under `miituber.onboardingTour.v1`, and the overlay is hidden from clean-output/capture-isolate modes. The dialogue is styled as a Wii-like speech bubble with the Mii portrait standing beside it. Portraits are ten pre-rendered 256px transparent PNGs in `public/tour/`, generated from `mee.ffsd` by `scripts/capture-tour-portraits.mjs` (drives the dev-only `?capture-portraits` page in headed Edge over raw CDP; headless WebGL hangs on this machine).
